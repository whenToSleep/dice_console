...
def start_bot():
    ...
    while start_message != "/start":
        ...
        if start_message != '/start' :
            print('Введи команду /start, пожалуйста. Или решил примерить роль QA?))')
    ...
    while True:
        ...
        try:
            ...
            if dice_message[0] == "/dice":
                ...
                else: print(f"Ты не можешь поставить больше, чем есть у второго игрока. Максимальная ставка на данный момент {int(balances['player2'])}")
...
def game_over(balances, result):
    if balances["player1"] > balances["player2"]:
        ...
        print(f"Ваши результаты: Побед - {result["player1"]}, Баланс - {balances["player1"]:.0f}")
        print(f"Банк заработал {balances["bank"]:.2f}")
